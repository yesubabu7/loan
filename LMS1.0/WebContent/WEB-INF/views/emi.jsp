            ...
            <tbody>
                <c:forEach items="${loanList}" var="loan">
                    <c:if test="${loan.customerId == customerId}">
                        ...
                        <c:set var="emiStartDate" value="${loan.applicationDate}" />
<c:set var="emiStartDateMillis" value="${emiStartDate.time}" />
<c:set var="emiStartDateIncrementedMillis" value="${emiStartDateMillis}" />

<c:forEach var="i" begin="1" end="${loan.noOfMonths}">
    <c:set var="emiStartDateIncrementedMillis" value="${emiStartDateIncrementedMillis + 30 * 24 * 60 * 60 * 1000}" />
    <c:set var="emiStartDate" value="${new java.sql.Timestamp(emiStartDateIncrementedMillis)}" />

    <!-- Use emiStartDate here for each month -->
    <tr>
        <td>${emiStartDate}</td>
        <!-- Other columns -->
    </tr>
